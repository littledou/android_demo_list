...
public class ClipZoomImageView extends ImageView implements OnScaleGestureListener, OnTouchListener,ViewTreeObserver.OnGlobalLayoutListener{
	...
	public boolean onScale(ScaleGestureDetector detector){
		...
		scaleRectF(rectF, scaleFactor);
		float max = (rectF.right-rectF.left)>(rectF.bottom-rectF.top)?(rectF.right-rectF.left):(rectF.bottom-rectF.top);
		if(max <= getWidth()&& scaleFactor < 1.0f){
			scaleFactor=1;
		}
		mScaleMatrix.postScale(scaleFactor, scaleFactor,detector.getFocusX(), detector.getFocusY());
		checkBorder();
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (isCanDrag)
			{
				if (getDrawable() != null)
				{
					...
					RectF rectF = getMatrixRectF();
					// 如果宽度小于屏幕宽度，则禁止左右移动
					rectF.offset(dx/100, dy/100);
					if((rectF.right-rectF.left>=getWidth())&&(rectF.bottom-rectF.top>=getWidth())){
						...
						if((rectF.right-rectF.left<=getWidth())&&(rectF.right>=getWidth()-mHorizontalPadding||rectF.left<=mHorizontalPadding)){
							dx = 0;
						}
						if((rectF.bottom-rectF.top<=getWidth())&&((rectF.bottom>=(getHeight() - mVerticalPadding))||
	...
	private void checkBorder()
	{
		...
		// 如果宽或高小于屏幕，则控制范围 ; 这里的0.001是因为精度丢失会产生问题，但是误差一般很小，所以我们直接加了一个0.01
		if (rect.width() + 0.01 <= width - 2 * mHorizontalPadding){
			if (rect.left < mHorizontalPadding){
				deltaX = -rect.left + mHorizontalPadding;
			}
